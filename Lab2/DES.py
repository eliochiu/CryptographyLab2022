...
def DES():
    ...
    mode = int(input())
    if mode == 1:
        plaintext = __Hexadecimal_to_Binary_String(text)
        initial_key = __Hexadecimal_to_Binary_String(key)
        while n != 0:
            plaintext = DES_Encryption(plaintext,initial_key)
            n = n - 1
        print(__Binary_String_to_Hexadecimal(plaintext))
    elif mode == 0:
        ciphertext = __Hexadecimal_to_Binary_String(text)
        initial_key = __Hexadecimal_to_Binary_String(key)
        while n != 0:
            ciphertext = DES_Decryption(ciphertext,initial_key)
            n = n - 1
        print(__Binary_String_to_Hexadecimal(ciphertext))
